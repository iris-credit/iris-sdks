...
- Fetchers accept a viem `Client` and return core-sdk classes, e.g. `fetchLoan(pod, client)`; resolve missing chain ids with `parameters.chainId ?? (await getChainId(client))`, then narrow via `ChainUtils.isSupportedChainId`.
