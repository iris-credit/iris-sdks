...
/**
 * JS implementation of {@link https://github.com/morpho-org/morpho-blue-irm/blob/main/src/adaptive-curve-irm/libraries/ExpLib.sol ExpLib} used by the Adaptive Curve IRM.
 *
 * Ported from {@link https://github.com/morpho-org/sdks/blob/main/packages/blue-sdk/src/math/AdaptiveCurveIrmLib.ts blue-sdk}.
 */
export namespace AdaptiveCurveIrmLib {
   ...
   * @example
   * ```ts
   * import { AdaptiveCurveIrmLib } from "@iris-credit/core-sdk";
   *
  ...
  export function getBorrowRate(
    ...
    const errNormFactor =
      startUtilization > TARGET_UTILIZATION ? MathLib.WAD - TARGET_UTILIZATION : TARGET_UTILIZATION;
    ...
    if (startRateAtTarget === 0n) {
      ...
      if (linearAdaptation === 0n) {
        ...
        const newRateAtTarget = (adaptation: bigint) =>
          MathLib.min(
            MathLib.max(MathLib.wMulDown(startRateAtTarget, wExp(adaptation)), MIN_RATE_AT_TARGET),
            MAX_RATE_AT_TARGET,
          );
        ...
        avgRateAtTarget =
          (startRateAtTarget + endRateAtTarget + 2n * newRateAtTarget(linearAdaptation / 2n)) / 4n;
      }
    }
