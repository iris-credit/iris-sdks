import type { PeripheralBalance } from "./AssetBalances.js";

import { describe, expect, test } from "vitest";
import { CHAIN_ADDRESSES, NATIVE_ADDRESS } from "../../addresses.js";
import { ChainId } from "../../chain.js";
import { Token } from "../token/Token.js";
import { AssetBalances } from "./AssetBalances.js";
...
describe("AssetBalances", () => {
  ...
  test("add accumulates into an existing allocation", () => {
    ...
    ab.add({ type: "wrapped", srcToken: WSTETH, srcAmount: 5n, dstAmount: 10n });
  ...
  test("sub decrements total and updates allocation", () => {
    ...
    ab.sub({ type: "wrapped", srcToken: WSTETH, srcAmount: 5n, dstAmount: 30n });
  ...
  test("add returns the same instance (chainable)", () => {
    ...
    expect(ab.add({ type: "vault", srcToken: ETH, srcAmount: 1n, dstAmount: 1n })).toBe(ab);
  });
  ...
  test("sub returns the same instance (chainable)", () => {
    ...
    expect(ab.sub({ type: "vault", srcToken: ETH, srcAmount: 1n, dstAmount: 1n })).toBe(ab);
  });
