...
describe("MathLib.maxUint", () => {
  ...
  test("error: invalid bit length", () => {
    expect(() => MathLib.maxUint(7)).toThrow("Invalid number of bits: 7");
    expect(() => MathLib.maxUint(33)).toThrow("Invalid number of bits: 33");
  });

  test("error: zero bit length", () => {
    // Nibble-aligned, so it passes the guard and fails on `BigInt("0x")` instead.
...
describe("MathLib.min / MathLib.max", () => {
  ...
  test("behavior: returns the single value of a one-element call", () => {
...
describe("MathLib.mulDivDown / MathLib.mulDivUp", () => {
  ...
  test("behavior: rounds either way around a repeating quotient", () => {
    // 10 / 3 = 3.33…
    expect(MathLib.mulDivDown(10n, 1n, 3n)).toBe(3n);
    expect(MathLib.mulDivUp(10n, 1n, 3n)).toBe(4n);
  });
  ...
  test("error: division by zero", () => {
    expect(() => MathLib.mulDivDown(1n, 1n, 0n)).toThrow("MathLib: DIVISION_BY_ZERO");
    expect(() => MathLib.mulDivUp(1n, 1n, 0n)).toThrow("MathLib: DIVISION_BY_ZERO");
  });
});
...
describe("MathLib.wDiv / MathLib.wDivDown / MathLib.wDivUp", () => {
  ...
  test("error: division by zero", () => {
    expect(() => MathLib.wDivDown(1n, 0n)).toThrow("MathLib: DIVISION_BY_ZERO");
  });
});
