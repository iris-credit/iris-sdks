...
/**
 * Computes classic ERC-20 approval transactions for GeneralAdapter1 or Permit2, given the
 * existing allowance.
 *
...
export const getRequirementsApproval = (params: {
  ...
  if (allowances < spendAmount) {
    if (APPROVE_ONLY_ONCE_TOKENS[chainId]?.includes(address) && allowances > 0n) {
      approvals.push(
        encodeErc20Approval({
          token: address,
          spender,
