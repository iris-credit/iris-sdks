...
import { CHAIN_ID, COLLATERAL_TOKEN, POD } from "../../../test/fixtures/iris.js";
...
import { irisSupplyCollateral } from "./supplyCollateral.js";
...
describe("irisSupplyCollateral", () => {
  test("default: pulls the collateral and supplies the funded total", () => {
    const tx = irisSupplyCollateral({
      chainId: CHAIN_ID,
      args: { pod: POD, token: COLLATERAL_TOKEN, amount: 1_000n },
    });

    expect(tx.to).toBe(bundler3);
    expect(tx.value).toBe(0n);
    expect(tx.action.type).toBe("irisSupplyCollateral");
    ...
    expect(callNames(calls)).toEqual(["erc20TransferFrom", "irisSupplyCollateral"]);
  ...
  test("error: NegativeInputError when amount is negative", () => {
    expect(() =>
      irisSupplyCollateral({
        chainId: CHAIN_ID,
        args: { pod: POD, token: COLLATERAL_TOKEN, amount: -1n },
      }),
  ...
  test("error: NegativeInputError when nativeAmount is negative", () => {
    expect(() =>
      irisSupplyCollateral({
        chainId: CHAIN_ID,
        args: { pod: POD, token: wNative, nativeAmount: -1n },
      }),
    ).toThrow(NegativeInputError);
  });
  ...
  test("error: NonPositiveInputError when nothing is funded", () => {
    expect(() =>
      irisSupplyCollateral({
        chainId: CHAIN_ID,
        args: { pod: POD, token: COLLATERAL_TOKEN, amount: 0n },
      }),
