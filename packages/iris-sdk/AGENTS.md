...
- Actions are pure encoders returning deep-frozen `Transaction<TAction>` with a typed `action` discriminator — no RPC, no clock, no signing — composing bundles in on-chain execution order with explicit `skipRevert` args. `action.args` carries only primitives or copies, never a caller-owned object reference — `deepFreeze` freezes in place, so an embedded reference would freeze the caller's object.
