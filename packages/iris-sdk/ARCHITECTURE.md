...
- **After resolution.** The pod may still hold a venue position (repay leaves the collateral with it). `escape` exits it: the borrower settles any remaining venue debt and withdraws the venue collateral, yield included. `repayEscape` fuses the two legs into one bundle.
...
`take`, `repay`, `repayEscape`, `supplyCollateral`, `supplyBond`, `escape` and `refinance` are routed through **Bundler3** via its **general adapter** (`GeneralAdapter1`). The bundle atomically:
...
| `repayEscape`        | Bundler3 (general adapter) | `repay` then `escape` in one bundle: Iris's repay leaves the collateral on the venue. Borrower only.             |
