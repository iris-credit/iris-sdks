...
`signResponse` derives the Permit2 payload from the quote itself (`solver`, `debtToken`, `bond`), so it can never mismatch the quote it accompanies, and scopes its expiration to `quote.deadline` — an unfilled quote's permit dies with the quote. Requirements cover allowances only: gate quoting on the solver's debt-token balance separately, and re-check as takes consume it.
