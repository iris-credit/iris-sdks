  ...
  afterAll(async () => {
    ...
    if (failures.length > 0)
      throw new AnvilCleanupError(
        ...
        {
          cause: new AggregateError(failures, "One or more Vitest Anvil cleanups failed."),
        },
      );
            ...
            try {
              ...
              if (cleanupError instanceof AnvilProcessError)
                ...
                throw createAnvilFailureCleanupError({
                  ...
                  summary: "Vitest fixture setup and Anvil cleanup both failed.",
                });
            }
          }

          if (error instanceof AnvilCleanupError || signal?.aborted || attempt === maxAttempts)
            break;
