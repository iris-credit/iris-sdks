...
const kebabCase = (str: string): string => str.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase();
...
function toArgs(obj: AnvilArgs) {
  return Object.entries(obj).flatMap<string>(([key, value]) => {
    ...
    if (Array.isArray(value)) return [`--${kebabCase(key)}`, value.join(",")];

    if (typeof value === "object" && value !== null) {
      return Object.entries(value).flatMap(([subKey, subValue]) => {
        if (subValue === undefined) return [];

        const flag = `--${kebabCase(`${key}.${subKey}`)}`;
...
export const spawnAnvil = async (
  ...
  const { binary = "anvil", redactForkUrl: redactForkUrlOption, ...anvilArgs } = args;
  const shouldRedactForkCredentials = redactForkUrlOption ?? process.env.CI === "true";
  ...
  for (const value of Object.values(args.forkHeader ?? {})) {
    if (value !== "" && !diagnosticRedactions.some((redaction) => redaction.value === value))
      diagnosticRedactions.push({
  ...
  const formatAnvilDiagnostics = (diagnostics: string) => {
    if (!shouldRedactForkCredentials || diagnosticRedactions.length === 0) return diagnostics;
    ...
    while (cursor < diagnostics.length) {
      ...
      formatted.push(diagnostics.slice(plainStart, cursor), redaction.replacement);
  ...
  const formatAnvilCause = (cause: unknown) => {
    if (!shouldRedactForkCredentials || diagnosticRedactions.length === 0) return cause;
    ...
    for (const property of Reflect.ownKeys(formattedCause)) {
      ...
      if (typeof value === "string")
        ...
        Reflect.set(
          formattedCause,
          property,
          value.map((item) => (typeof item === "string" ? formatAnvilDiagnostics(item) : item)),
        );
    }
  ...
  try {
    ...
    let processCloseTimeout: ReturnType<typeof globalThis.setTimeout> | undefined;
    ...
    const stopProcess = () => {
      ...
      if (subprocess.exitCode === null && subprocess.signalCode === null) {
        ...
        try {
          ...
          console.warn("Failed to send SIGINT to Anvil.", formatAnvilCause(error));
        }
        ...
        if (forceKillAfterMs !== false) {
          forceKillTimeout = globalThis.setTimeout(() => {
            ...
            try {
              ...
              if (!forceKillSent) console.warn("Failed to send SIGKILL to Anvil after timeout.");
            } catch (error) {
              console.warn(
          ...
          if (typeof forceKillTimeout === "object" && "unref" in forceKillTimeout) {
      ...
      if (processCloseTimeoutMs !== undefined) {
        ...
        if (typeof processCloseTimeout === "object" && "unref" in processCloseTimeout) {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const stderrLineBufferLength = ANVIL_DIAGNOSTICS_MAX_LENGTH + diagnosticCarryLength;
      ...
      subprocess.stdout.on("data", (data) => {
        ...
        const listenMatch = stdout.match(/Listening on 127\.0\.0\.1:(\d+)\r?\n/);
      ...
      subprocess.stderr.on("data", (data) => {
        ...
        if (!shouldRedactForkCredentials || diagnosticRedactions.length === 0) {
          stderr = `${stderr}${dataString}`.slice(-ANVIL_DIAGNOSTICS_MAX_LENGTH);
        } else {
          ...
          stderr = `${stderr}${formatAnvilDiagnostics(rawDiagnostics.slice(0, cursor))}`.slice(
      ...
      subprocess.once("close", (code, signal) => {
        ...
        const expectedShutdown =
          stopRequested &&
          (code === 0 ||
            (code === null && (signal === "SIGINT" || (signal === "SIGKILL" && forceKillSent))));
        const processExitError =
          ...
          listening && (processError !== undefined || !expectedShutdown)
            ...
            : undefined;
        if (forceKillTimeout !== undefined) globalThis.clearTimeout(forceKillTimeout);
        if (processCloseTimeout !== undefined) globalThis.clearTimeout(processCloseTimeout);
