...
describe("expectReadCall", () => {
  ...
  test("filters out unrelated calls to the same address before decoding", async () => {
    ...
    const unrelatedAbi = parseAbi(["function totalSupply() view returns (uint256)"]);
  ...
  test("returns empty array when the caller passes a wrong-arity ABI for the same name", async () => {
    ...
    const wrongArityAbi = parseAbi(["function balanceOf(uint256) view returns (uint256)"]);
    expect(
      expectReadCall(handle, {
        address: TOKEN,
        abi: wrongArityAbi,
