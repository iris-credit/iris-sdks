...
import { AnvilCleanupError, AnvilProcessError, AnvilStartupError } from "./errors.js";
...
describe.sequential("spawnAnvil", () => {
  test("behavior: tolerates stderr output while Anvil is starting", async () => {
    ...
    spawnMock.mockReturnValue(subprocess as unknown as ChildProcessWithoutNullStreams);
    ...
    expect(spawnMock.mock.calls[0]?.[1]).not.toContain("--compute-units-per-second");
  ...
  test("error: stop reports an undelivered state-dump signal", async () => {
    ...
    try {
      ...
      await expect(spawned.stopAndWait()).rejects.toBeInstanceOf(AnvilCleanupError);
      expect(subprocess.kill).toHaveBeenCalledOnce();
      expect(subprocess.kill).toHaveBeenCalledWith("SIGINT");
  ...
  test("behavior: redacts exact fork URL and header values from CI diagnostics", async () => {
    ...
    try {
      ...
      subprocess.stderr.write(`provider request failed for ${forkUrl.slice(0, forkUrlSplit)}`);
  ...
  test("error: AnvilProcessError surfaces an unexpected post-startup exit", async () => {
    ...
    const error = await spawned.stopAndWait().catch((caught: unknown) => caught);
  ...
  test("error: AnvilProcessError preserves post-startup process errors", async () => {
    ...
    expect(() => subprocess.emit("error", new Error("second process error"))).not.toThrow();
    subprocess.exitCode = 0;
  ...
  test.each([
    ...
    async ({ code, signal }) => {
      ...
      const failure = expect(spawned.stopAndWait()).rejects.toBeInstanceOf(AnvilProcessError);
      expect(subprocess.kill).not.toHaveBeenCalled();
  ...
  test("error: AnvilStartupError survives a nonzero close after a subprocess error", async () => {
    const forkUrl = "https://rpc.example/v1/project-id";
    const subprocess = createFakeAnvilProcess({ closeOnSignal: false });
    const subprocessError = Object.assign(new Error(`failed to spawn --fork-url ${forkUrl}`), {
      code: "ENOENT",
  ...
  test("error: AnvilStartupError redacts fork credentials from subprocess causes in CI", async () => {
    ...
    const subprocessError = Object.assign(
      new Error(`failed to spawn --fork-url ${forkUrl} --fork-header ${forkHeader}`),
      {
        code: "ENOENT",
  ...
  test("error: AnvilStartupError redacts the fork URL in stderr diagnostics", async () => {
    ...
    const error = await spawnedPromise.catch((caught: unknown) => caught);
    expect(error).toBeInstanceOf(AnvilStartupError);
    expect(String(error)).toContain("provider request failed for <redacted-fork-url>");
  ...
  test("error: AnvilStartupError uses the configured startup deadline", async () => {
    ...
    const spawnedPromise = spawnAnvil({ chainId: 1 }, { startupTimeoutMs: 1_000 });
    const rejection = expect(spawnedPromise).rejects.toBeInstanceOf(AnvilStartupError);
  ...
  test("error: AnvilStartupError rejects a pre-aborted launch", async () => {
    ...
    const error = await spawnAnvil({ chainId: 1 }, { signal: controller.signal }).catch(
  ...
  test("error: AnvilStartupError aborts after launch but before listening", async () => {
    ...
    const spawnedPromise = spawnAnvil({ chainId: 1 }, { signal: controller.signal });
    await vi.waitFor(() => expect(spawnMock).toHaveBeenCalledOnce());
    controller.abort(abortReason);
  ...
  test.each([{ dumpState: "/tmp/anvil-state.json" }, { state: "/tmp/anvil-state.json" }])(
    "behavior: does not force-kill state dumps",
    async (args) => {
      const subprocess = createFakeAnvilProcess({ closeOnSignal: false });
      spawnMock.mockReturnValue(subprocess as unknown as ChildProcessWithoutNullStreams);
