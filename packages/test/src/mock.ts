// This module imports `vi` from `vitest` at the top level. That's intentional:
// `@iris-credit/test`'s `vitest` peer is *optional*, so any production codebase that does
// not have vitest installed will fail to resolve `@iris-credit/test/mock` at import time —
// which is exactly what we want, since the mock client is for tests only and must not be
// loaded in production runtimes. Do not "tree-shake" or move this import.
import type {
  Abi,
  Address,
  Chain,
 ...
 * ```
 */
export function createMockClient<chain extends Chain>(chain: chain): MockClientHandle<chain> {
  ...
  const request = vi.fn<RpcHandler>(async ({ method, params }) => {
    ...
    throw new Error(`[createMockClient] unhandled RPC ${method} ${JSON.stringify(params)}`);
  });
  const client = createClient({ chain, transport: custom({ request }) });
  return { client, request, chain, dispatch };
}

interface MockReadOptions<abi extends Abi, fn extends ContractFunctionName<abi, "view" | "pure">> {
  address: Address;
  abi: abi;
  functionName: fn;
  result: unknown;
 ...
 * ```
 */
export function mockRead<abi extends Abi, fn extends ContractFunctionName<abi, "view" | "pure">>(
  handle: MockClientHandle,
  options: MockReadOptions<abi, fn>,
): void {
  ...
  if (fnAbiItems.length === 0)
    throw new Error(`[mockRead] function ${String(options.functionName)} not found in abi`);
  ...
  for (const item of fnAbiItems) {
    const selector = toFunctionSelector(toFunctionSignature(item)).toLowerCase();
    try {
 ...
 * ```
 */
export function expectReadCall<abi extends Abi, fn extends ContractFunctionName<abi>>(
  ...
  const selectors = new Set(
    match.abi
      ...
      .map((item) => toFunctionSelector(toFunctionSignature(item)).toLowerCase()),
  );
