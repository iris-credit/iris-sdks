 ...
 */
export function validateAuthorizations(authorizations: SimulationAuthorization[]): string[] {
  const errors: string[] = [];
  ...
  for (let i = 0; i < authorizations.length; i++) {
    ...
    if (auth.type === "signature") {
      if (!auth.token || auth.token === zeroAddress) {
        errors.push(`authorizations[${i}]: signature authorization has zero-address token`);
      }
      if (!auth.spender || auth.spender === zeroAddress) {
        errors.push(`authorizations[${i}]: signature authorization has zero-address spender`);
      }
    }
  }
