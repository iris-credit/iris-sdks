...
import { CHAIN_ADDRESSES, getChainAddresses } from "@iris-credit/core-sdk";

vi.mock("@iris-credit/core-sdk", async (importOriginal) => {
  const mod = await importOriginal<typeof import("@iris-credit/core-sdk")>();
...
// Pulled from core-sdk so tests exercise the real Set membership check.
...
describe("assertNoBundlerRetention", () => {
  ...
  it("does not throw for dust amounts to blacklisted addresses", () => {
    const transfers = parseTransfers([
      makeCall([makeTransferLog({ token: USDC, from: USER, to: VAULT, amount: 50n })]),
    ]);
    expect(() =>
      assertNoBundlerRetention({ chainId: 1, transfers, assetChanges: [] }),
    ).not.toThrow();
  ...
  it("warns and skips when core-sdk knows the chain but has no bundler3 config", () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      "Chain known to core-sdk but has no bundler3 config, retention check skipped",
      { chainId: 1 },
    );
  });
  ...
  it("propagates unexpected SDK errors instead of swallowing them", () => {
    ...
    expect(() => assertNoBundlerRetention({ chainId: 1, transfers, assetChanges: [] })).toThrow(
      "unexpected SDK bug",
    );
  });
  ...
  it("BlacklistViolationError.assetChanges includes {address, token, netRetained}", () => {
    const transfers = parseTransfers([
      makeCall([makeTransferLog({ token: USDC, from: USER, to: BUNDLER, amount: 777n })]),
    ]);
  ...
  it("behavior: throws when bundler retains native ETH reported only in assetChanges (Tenderly path)", () => {
    ...
    expect(() =>
      assertNoBundlerRetention({
        chainId: 1,
        transfers: [],
        assetChanges: [{ account: BUNDLER, changes: [{ token: ethAddress, diff: ONE_ETH }] }],
      }),
  ...
  it("behavior: counts native ETH once when present in both a transfer log and assetChanges (eth_simulateV1 path)", () => {
    ...
    const transfers = [{ token: ethAddress, from: USER, to: BUNDLER, amount: ONE_ETH, txIdx: 0 }];
    try {
      assertNoBundlerRetention({
        chainId: 1,
        transfers,
  ...
  it("behavior: does not throw when native ETH passes through the bundler (net zero)", () => {
    expect(() =>
      assertNoBundlerRetention({
        chainId: 1,
        transfers: [],
        assetChanges: [{ account: BUNDLER, changes: [{ token: ethAddress, diff: 0n }] }],
      }),
    ).not.toThrow();
  });
  ...
  it("behavior: does not throw for native ETH retention below dust threshold", () => {
    expect(() =>
      assertNoBundlerRetention({
        chainId: 1,
        transfers: [],
        assetChanges: [{ account: BUNDLER, changes: [{ token: ethAddress, diff: 50n }] }],
      }),
    ).not.toThrow();
  });
  ...
  it("behavior: ignores native ETH assetChanges for non-bundler accounts", () => {
    expect(() =>
      assertNoBundlerRetention({
        chainId: 1,
        transfers: [],
        assetChanges: [{ account: VAULT, changes: [{ token: ethAddress, diff: ONE_ETH }] }],
      }),
    ).not.toThrow();
  });
  ...
  it("behavior: reports both ERC20 (from logs) and native ETH (from assetChanges) retention together", () => {
    ...
    try {
      ...
      expect(tokens).toEqual([USDC.toLowerCase(), ethAddress.toLowerCase()].sort());
    }
  });
});
