...
import type { AccountAssetChanges, SimulationLogger, Transfer } from "../../types.js";
...
function getBundlerAddresses(chainId: number, logger?: SimulationLogger): Set<Address> {
  try {
    const addresses = getChainAddresses(chainId as ChainId);
    if (!addresses.bundler3) {
      // core-sdk knows the chain but didn't catalog a bundler3 for it.
      // Treat the same as UnsupportedChainIdError — retention check skipped.
      logger?.warn("Chain known to core-sdk but has no bundler3 config, retention check skipped", {
        chainId,
      });
      return new Set();
    }
    return new Set(
      Object.values<Address>(addresses.bundler3)
        .filter(isDefined)
    ...
    if (error instanceof UnsupportedChainIdError) {
      ...
      logger?.warn("Chain not supported by core-sdk, retention check skipped", {
        chainId,
      });
      return new Set();
    }
 ...
 */
export function assertNoBundlerRetention(params: AssertNoBundlerRetentionParams): void {
  ...
  const recordFlow = (address: Address, token: Address, delta: bigint): void => {
  ...
  if (drained.length > 0) {
    logger?.warn("Simulation detected pre-existing bundler balance being swept", {
