...
describe("buildSimulationTxs", () => {
  ...
  it("prepends an approval-type authorization tx as-is", () => {
    ...
    const auths: SimulationAuthorization[] = [{ type: "approval", transaction: approvalTx }];
  ...
  it("preserves order: all auths first, then all user txs", () => {
    ...
    const result = buildSimulationTxs(makeParams({ authorizations: auths, transactions: userTxs }));
  ...
  it("rejects approval-type auth whose `from` does not match the user sender (defense-in-depth)", () => {
    const DIFFERENT_SENDER: Address = "0x5555555555555555555555555555555555555555";
    const auths: SimulationAuthorization[] = [
      {
        type: "approval",
        transaction: {
    ...
    expect(() => buildSimulationTxs(makeParams({ authorizations: auths }))).toThrow(
