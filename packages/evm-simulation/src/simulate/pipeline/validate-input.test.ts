...
describe("validateInput", () => {
  ...
  it("throws when transactions is empty", () => {
    expect(() => validateInput(params({ transactions: [] }))).toThrow(SimulationValidationError);
  });

  it("throws when transactions is omitted", () => {
    expect(() => validateInput(params({ transactions: undefined as never }))).toThrow(
  ...
  it("throws when a tx has empty data", () => {
    expect(() =>
      validateInput(params({ transactions: [{ from: USER, to: VAULT, data: "" as Hex }] })),
    ).toThrow(SimulationValidationError);
  });
  ...
  it("collects multiple errors in fieldErrors[]", () => {
    try {
      validateInput(
        params({
          transactions: [{ from: zeroAddress, to: zeroAddress, data: "" as Hex }],
        }),
      );
  ...
  it("runs validateAuthorizations and includes its errors", () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(SimulationValidationError);
      const fieldErrors = (err as SimulationValidationError).fieldErrors ?? [];
      expect(fieldErrors.some((e) => e.includes("zero-address token"))).toBe(true);
    }
  });
  ...
  it("does not throw when signature authorizations are valid", () => {
    ...
    expect(() => validateInput(params({ authorizations: auths }))).not.toThrow();
  });
  ...
  it("throws when a tx has negative value", () => {
    expect(() =>
      validateInput(
        params({
          transactions: [{ from: USER, to: VAULT, data: "0x12" as Hex, value: -1n }],
        }),
      ),
    ).toThrow(SimulationValidationError);
  });
