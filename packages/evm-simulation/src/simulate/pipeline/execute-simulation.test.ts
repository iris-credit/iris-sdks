...
const txs: SimulationTransaction[] = [{ from: USER, to: VAULT, data: "0x12" as Hex }];
...
describe.sequential("executeSimulation — Tenderly + simulateV1 configured", () => {
  ...
  it("falls back to simulateV1 when Tenderly throws ExternalServiceError", async () => {
    ...
    expect(mockSimulateV1).toHaveBeenCalledWith(expect.objectContaining({ wNative: WETH }));
  });

  it("does NOT fall back when Tenderly throws SimulationRevertedError", async () => {
    mockTenderlyRpc.mockRejectedValueOnce(new SimulationRevertedError("reverted"));
