...
function makeConfig(chains: Array<[number, ChainSimulationConfig]>): SimulationConfig {
...
describe("resolveChain", () => {
  it("returns both backends when both are configured", () => {
    const cap = resolveChain(
      makeConfig([[1, { tenderlyRpc: TENDERLY, simulateV1Url: "http://rpc.local" }]]),
      1,
    );
  ...
  it("returns only simulateV1Url when Tenderly is absent", () => {
    const cap = resolveChain(makeConfig([[137, { simulateV1Url: "http://poly" }]]), 137);
  ...
  it("throws UnsupportedChainError when the chainId is missing from the map", () => {
    expect(() => resolveChain(makeConfig([[1, { tenderlyRpc: TENDERLY }]]), 999999)).toThrow(
