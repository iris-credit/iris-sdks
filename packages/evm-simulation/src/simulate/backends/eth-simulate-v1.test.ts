...
describe.sequential("simulateV1", () => {
    ...
    mockSimulateCalls.mockResolvedValueOnce({
      results: [
        ...
        },
        {
          ...
          gasUsed: 42_000n,
          data: "0x" as Hex,
          logs: [{ address: USDC, topics: ["0xbbbb" as Hex], data: "0xabcd" as Hex }],
        },
      ],
    });
  ...
  it("rejects bundles where transactions have different senders", async () => {
    await expect(
      simulateV1({
        rpcUrl: "http://rpc.local",
        chainId: 1,
        transactions: [BASIC_TX, { from: OTHER, to: VAULT, data: "0x12" as Hex }],
      }),
    ).rejects.toThrow(SimulationValidationError);
  });
  ...
  it("inflates sender ETH balance to half of uint256 via stateOverride", async () => {
    mockSimulateCalls.mockResolvedValueOnce({
      results: [{ status: "success", gasUsed: 0n, data: "0x" as Hex, logs: [] }],
    });
    ...
    expect(callArgs.stateOverrides).toEqual([{ address: USER, balance: maxUint256 / 2n }]);
  });
  ...
  it("preserves SimulationRevertedError when viem throws one", async () => {
    mockSimulateCalls.mockRejectedValueOnce(new SimulationRevertedError("revert"));
