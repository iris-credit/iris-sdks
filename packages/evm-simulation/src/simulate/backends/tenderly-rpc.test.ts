...
describe.sequential("simulateTenderlyRpc — single tx", () => {
  ...
  it("groups assetChanges by account (both transfer endpoints)", async () => {
    ...
    expect(result.assetChanges).toEqual([
      {
        account: USER,
        changes: [{ token: USDC, symbol: "USDC", decimals: 6, diff: 1_000_000n }],
      },
      {
        account: VAULT,
        changes: [{ token: USDC, symbol: "USDC", decimals: 6, diff: -1_000_000n }],
  ...
  it("maps a checksummed native sentinel in contractAddress to the eth sentinel", async () => {
    // Tenderly may echo the native sentinel (checksummed) as an asset change's
    // contractAddress. It must still key native ETH by the exact `ethAddress`
    // constant so a bundler3 native residual is not silently dropped by
  ...
  it("inflates the sender ETH balance via state overrides to dodge insufficient-funds false reverts", async () => {
    ...
    const body = requestBody(fetchMock.mock.calls[0]!);
    const [, , overrides] = body.params as [unknown, unknown, Record<Address, { balance: Hex }>];
  ...
  it("defaults omitted log fields to empty topics and 0x data", async () => {
    ...
    expect(result.calls[0]!.logs).toEqual([{ address: USDC, topics: [], data: "0x" }]);
  });
...
describe.sequential("simulateTenderlyRpc — bundle", () => {
  it("dispatches tenderly_simulateBundle for >1 tx", async () => {
    ...
    const [txs] = body.params as [Array<{ to: Address; data: Hex; input: Hex }>, string];
  ...
  it("emits one call per bundle step with its own gas/output/logs", async () => {
    const fetchMock = vi.fn<MockFetch>().mockResolvedValueOnce({
      ...
      json: async () =>
        envelope([
          successResult({
            logs: [{ raw: { address: USDC, topics: ["0x1111" as Hex], data: "0x" } }],
            gasUsed: "0x2af8",
            output: "0x1111",
          }),
          successResult({
            logs: [{ raw: { address: USDC, topics: ["0x2222" as Hex], data: "0x" } }],
            gasUsed: "0x55f0",
            output: "0x2222",
          }),
        ]),
  ...
  it("nets assetChanges per account across all bundle steps", async () => {
    const fetchMock = vi.fn<MockFetch>().mockResolvedValueOnce({
      ...
      json: async () =>
        envelope([
          successResult({
            assetChanges: [assetChange({ token: USDC, to: USER, rawAmount: "0x3" })],
          }),
          successResult({
            assetChanges: [
  ...
  it("throws SimulationRevertedError when a bundle step has status=false", async () => {
    const fetchMock = vi.fn<MockFetch>().mockResolvedValueOnce({
      ok: true,
      json: async () => envelope([successResult(), revertResult(), successResult()]),
    });
    installFetchMock(fetchMock);
...
describe.sequential("simulateTenderlyRpc — errors", () => {
  it("rejects an empty transactions array with SimulationValidationError", async () => {
    await expect(simulateTenderlyRpc({ config: CONFIG, transactions: [] })).rejects.toThrow(
      SimulationValidationError,
    );
  });
  ...
  it("throws SimulationRevertedError when status is false", async () => {
    ...
    await expect(simulateTenderlyRpc({ config: CONFIG, transactions: [TX1] })).rejects.toThrow(
      SimulationRevertedError,
    );
  });
  ...
  it("throws ExternalServiceError when result is missing", async () => {
    ...
    await expect(simulateTenderlyRpc({ config: CONFIG, transactions: [TX1] })).rejects.toThrow(
      "Tenderly RPC returned no result",
    );
  });

  it("wraps unknown fetch errors in ExternalServiceError", async () => {
    const fetchMock = vi.fn<MockFetch>().mockRejectedValueOnce(new Error("ECONNREFUSED"));
    installFetchMock(fetchMock);
  ...
  it("passes AbortSignal to fetch", async () => {
    ...
    expect(requestInit(fetchMock.mock.calls[0]!).signal).toBe(controller.signal);
  });
  ...
  it("throws ExternalServiceError when a bundle returns an empty array", async () => {
    ...
    await expect(simulateTenderlyRpc({ config: CONFIG, transactions: [TX1, TX2] })).rejects.toThrow(
      ExternalServiceError,
    );
  });
});
