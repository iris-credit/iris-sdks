...
const addressSchema = z.custom<Address>((val) => typeof val === "string" && isAddress(val));
...
function rpcEnvelope<T extends z.ZodTypeAny>(result: T) {
  return z.object({ result: result.optional(), error: rpcErrorSchema.optional() }).passthrough();
}
...
export async function simulateTenderlyRpc(params: {
  ...
  if (!firstTx) {
    throw new SimulationValidationError("At least one transaction is required", []);
  }
...
function unwrapResult<T>(envelope: { result?: T; error?: { message: string } }): T {
  if (envelope.error) {
    throw new ExternalServiceError(`Tenderly RPC error: ${envelope.error.message}`);
  }
...
function buildStateOverrides(sender: Address): Record<Address, { balance: Hex }> {
...
function encodeBlock(blockNumber?: bigint | BlockTag): string {
  ...
  return typeof blockNumber === "bigint" ? numberToHex(blockNumber) : blockNumber;
}
