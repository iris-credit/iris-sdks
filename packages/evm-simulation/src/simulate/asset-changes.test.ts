...
describe("normalizeAssetToken", () => {
  ...
  test("behavior: collapses the checksummed native sentinel to ethAddress", () => {
    // Security invariant: a checksummed sentinel must still key native ETH by the
    // exact `ethAddress` constant, otherwise a bundler3 native residual lands on a
    // separate map key and escapes `assertNoBundlerRetention`.
...
describe("groupAssetChanges", () => {
  ...
  test("behavior: nets contributions for the same account and token", () => {
    ...
    expect(result).toEqual([{ account: USER, changes: [{ token: USDC, diff: 70n }] }]);
  });
  ...
  test("behavior: merges one account regardless of input casing", () => {
    ...
    expect(result).toEqual([{ account: USDC, changes: [{ token: ethAddress, diff: 3n }] }]);
  });
