...
describe.sequential("simulateV1 — assetChanges on a mainnet fork", () => {
  test("ERC20 transfer: sender debited and recipient credited the amount", async ({ client }) => {
  ...
  test("WETH unwrap: internal native-ETH refund is captured via traceTransfers", async ({
    ...
    const senderChanges = result.assetChanges.find((c) => c.account === client.account.address);
