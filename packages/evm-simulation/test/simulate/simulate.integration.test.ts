...
import { concatHex, encodeFunctionData, padHex, parseEther, toHex, zeroAddress } from "viem";
...
describe.sequential("simulate — registered wrapped-native events", () => {
  test("parses WETH9 events only from the registered contract", async ({ client }) => {
    ...
    expect(result.transfers.every(({ token }) => token !== LOOKALIKE_TOKEN)).toBe(true);
    expect(
