...
// `describe.sequential` is required because the tests below drive branching
// with `vi.stubGlobal("window"|"navigator"|"document", …)`, and the first test
// additionally asserts on the un-stubbed Node global. These stubs mutate Node's
// process-global namespace, so two concurrent tests would overwrite each
// other's globals (and one's afterEach `vi.unstubAllGlobals()` could revoke
// another's stubs mid-flight).
...
describe("convertNumStrToLocal", () => {
  ...
  test("handles values with multiple group separators", () => {
    expect(convertNumStrToLocal("1,234,567.89", "en-US", "de-DE")).toBe("1.234.567,89");
  });
