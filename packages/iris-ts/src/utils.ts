...
export const isHexEqual = (left: `0x${string}`, right: `0x${string}`) =>
...
// eslint-disable-next-line no-explicit-any -- recursion breaks type
...
export const hasValue = <T, Path extends PartialDottedKeys<T> = PartialDottedKeys<T>>(
  data: T,
  path: Path,
...
export const getValue = <T, Path extends PartialDottedKeys<T> = PartialDottedKeys<T>>(
  data: T,
  path: Path,
...
export const keys = <T>(o?: T) =>
  Object.keys(o ?? {}) as (T extends ArrayLike<unknown>
    ? `${number}`
    : `${Extract<keyof T, number>}` | Extract<keyof T, string>)[];
...
export function deepFreeze<T>(obj: T): T {
  ...
  for (const name of propNames) {
    // eslint-disable-next-line no-explicit-any -- name is a property of obj
