...
describe("Time — unit conversion matrix", () => {
  ...
  describe("large values", () => {
    ...
    test("survives extremely large bigints without precision loss", () => {
      expect(Time.s.from.y(10_000_000_000n)).toBe(10_000_000_000n * 365n * 24n * 60n * 60n);
    });
    test("ms → s truncates the sub-second remainder of a large bigint", () => {
      expect(Time.s.from.ms(123456789999n)).toBe(123456789n);
    });
    test("s → ms scales a large bigint", () => {
      expect(Time.ms.from.s(123456789444n)).toBe(123456789444000n);
    });
  });
});
...
describe("Time.wait", () => {
  ...
  test("resolves after a 1 second delay", async () => {
    const start = Date.now();
