...
function formatBI(
  ...
  return (
    (isNegative && !/^0\.0+$/.test(value) ? "-" : formatOptions.sign ? "+" : "") +
    _withUnit(value, formatOptions.unit)
  );
}

type FormatterWithDefault<F extends BaseFormatter> = {
  of(value: bigint | null | undefined, decimals: number | null | undefined): string;
...
export abstract class BaseFormatter {
  ...
  of(value: bigint | number | null | undefined, decimals?: number | null | undefined) {
...
>(defaultOptions: Partial<TDefaultOptions> = {}, customFormatters: TCustom = {} as TCustom) {
  const formatters = {
    get hex() {
