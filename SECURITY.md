...
The Iris SDK packages are used to build transactions that can move real funds on the Iris protocol. We take security reports seriously.

## Reporting a Vulnerability

Do not open a public GitHub issue for security reports.

Email **team@iris.credit** with:
...
Only the latest published versions of maintained `@iris-credit/*` SDK packages receive security fixes.
...
- Issues in `viem` or other third-party dependencies; report these upstream.
- Social-engineering, DoS on npm or GitHub, and physical attacks.

## Dependency Version Ranges

Runtime dependencies in package manifests, including `@iris-credit/*` packages, may use caret ranges. This lets coordinated patch and minor releases reach SDK consumers without requiring a release for every upstream patch.
...
- If you require stricter isolation, pin `@iris-credit/*` to exact versions in your own manifest, or use `overrides` or `resolutions` to pin transitive versions.
